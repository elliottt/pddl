...
module Text.PDDL.Location where
...
-- Drop the location information from a pretty-printed thing.
instance PP a => PP (Located a) where
  pp loc = pp (unLoc loc)

ppWithLoc :: PP a => Located a -> Doc
...
instance PP SrcLoc where
  pp (SrcLoc r mb) = ppSource mb <> char ':' <> pp r
  pp NoLoc         = empty
...
instance PP Position where
  pp pos = int (posLine pos) <> char ':' <> int (posCol pos)
